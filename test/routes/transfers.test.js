...
const TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MjAwMDAsIm5hbWUiOiJVc2VyICMxIiwibWFpbCI6InVzZXIxQG1haWwuY29tIn0.FTs2eAFqPlvLZcaskdDLRD2qRIyCvG8NIjEj9Jy_COY';
...
test('Deve inserir uma transferência com sucesso', () => {
  return request(app).post(MAIN_ROUTE)
    .set('authorization', `bearer ${TOKEN}`)
    .send({ description: 'Regular Transfer', user_id: 20000, acc_ori_id: 20000, acc_dest_id: 20001, ammount: 100, date: new Date() })
    .then(async (res) => {
      ...
      expect(transactions[0].description).toBe('Transfer to acc #20001');
      expect(transactions[1].description).toBe('Transfer from acc #20000');
      expect(transactions[0].ammount).toBe('-100.00');
      expect(transactions[1].ammount).toBe('100.00');
      expect(transactions[0].acc_id).toBe(20000);
      expect(transactions[1].acc_id).toBe(20001);
    });
});
...
describe('Ao salvar uma transferência válida...', () => {
  ...
  test('A transação de saída deve ser negativa', () => {
    expect(outcome.description).toBe('Transfer to acc #20001');
    expect(outcome.ammount).toBe('-100.00');
    expect(outcome.acc_id).toBe(20000);
    expect(outcome.type).toBe('O');
  });

  test('A transação de entrada deve ser positiva', () => {
    expect(income.description).toBe('Transfer from acc #20000');
    expect(income.ammount).toBe('100.00');
    expect(income.acc_id).toBe(20001);
...
describe('Ao tentar salvar uma transferência inválida...', () => {
  const validTransfer = { description: 'Regular Transfer', user_id: 20000, acc_ori_id: 20000, acc_dest_id: 20001, ammount: 100, date: new Date() };
  ...
  test('Não deve inserir se as contas de origem e destino forem as mesmas', () => template({ acc_dest_id: 20000 }, 'Não é possível transferir de uma conta para ela mesma'));
  test('Não deve inserir se as contas pertencerem a outro usuário', () => template({ acc_ori_id: 20002 }, 'Conta #20002 não pertence ao usuário'));
});
...
describe('Ao alterar uma transferência válida...', () => {
  ...
  test('Deve retornar o status 200 e os dados da transferência', () => {
    return request(app).put(`${MAIN_ROUTE}/20000`)
      .set('authorization', `bearer ${TOKEN}`)
      .send({ description: 'Transfer Updated', user_id: 20000, acc_ori_id: 20000, acc_dest_id: 20001, ammount: 500, date: new Date() })
      .then(async (res) => {
        expect(res.status).toBe(200);
...
describe('Ao remover uma transferência', () => {
  test('Deve retornar o status 204', () => {
    return request(app).delete(`${MAIN_ROUTE}/20000`)
      .set('authorization', `bearer ${TOKEN}`)
      .then((res) => {
  ...
  test('O registro deve ter sido removido do banco', () => {
    return app.db('transfers').where({ id: 20000 })
      .then((result) => {
  ...
  test('As teransaçõs associadas devem ter sido removidas', () => {
    return app.db('transactions').where({ transfer_id: 20000 })
      .then((result) => {
        expect(result).toHaveLength(0);
      });
  });
...
test('Não deve retornar transferência de outro usuário', () => {
  return request(app).get(`${MAIN_ROUTE}/20001`)
    .set('authorization', `bearer ${TOKEN}`)
    .then((res) => {
